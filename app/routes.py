...
@login_required
def profile():
    note_count = Note.query.filter_by(user_id=current_user.user_id).count()
    public_note_count = Note.query.filter_by(user_id=current_user.user_id, is_public=True).count()
    deck_count = Deck.query.filter_by(user_id=current_user.user_id).count()
    public_deck_count = Deck.query.filter_by(user_id=current_user.user_id, is_public=True).count()

    quiz_count = QuizSession.query.filter_by(user_id=current_user.user_id, is_saved=True).count()
    
    # avg quiz score
    quizzes = QuizSession.query.filter_by(user_id=current_user.user_id, is_saved=True).all()
    if quizzes:
